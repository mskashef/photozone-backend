...
const savePost = (username, postId, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const getUsernameOfUid = (uid, callback = () => {}, errCallback = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const addTagsToPost = (postId, tags, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const getUserInfo = (
  ...
  connection.connect();
  connection.query(
    queries.getFollowersCount(username),
    (error, rows, fields) => {
      if (error) err(error);
      else {
        userInfo.followersCount = rows[0].n;
        connection.query(
          queries.getFollowingsCount(username),
          (error, rows, fields) => {
            if (error) {
              err(error);
            } else {
              ...
              connection.query(
                queries.getPostsCount(username),
                (error, rows, fields) => {
                  if (error) err(error);
                  else {
                    userInfo.postsCount = rows[0].n;
                    connection.query(
                      queries.getUserInfo(username),
                      (error, rows, fields) => {
                        if (error) err(error);
                        else {
                          ...
                          connection.query(
                            queries.amIFollowingUser(myUsername, username),
                            (error, rows, fields) => {
                              if (error) err(error);
                              else {
...
const getPostsOfUser = (username, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const followOrUnfollow = (
  ...
  connection.connect();
  let query = follow
    ? queries.follow(username, following)
    : queries.unfollow(username, following);
  console.log(query);
  connection.query(query, (error, rows, fields) => {
    if (error) err(error);
    else callback();
...
const addPost = (
  ...
  connection.query(query, (error, rows, fields) => {
    if (error) err(error);
      ...
      addTagsToPost(postId, tags, callback, (error) => {
        err(error);
      });
  });
  connection.end();
...
const getTagsOfPost = (postId, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const getPost = (postId, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const searchPosts = (tags, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
...
const searchUsers = (searchText, callback = () => {}, err = () => {}) => {
  const connection = mySql.createConnection(config);
  connection.connect();
